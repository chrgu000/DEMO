...
{
    public class InvStockPrinterBLL
    {
...
select  cast(0 as bit) as IsSelected ,
        ...
        cast(c.iQuantity as decimal(18, 2)) as 单据数量 ,
        cast(0 as decimal(18, 2)) as 批量 ,
        cBatch as 批号 ,
        '' as 用料客户 ,
        '' as 料头责任人 ,
        c.cWhCode as 仓库编码 ,
        isnull(c.cFree1, '') as 变化宽幅 ,
        isnull(c.cFree2, '') as 变化长度 ,
...
from    dbo.CurrentStock C  with (nolock) 
        join inventory   with (nolock) on C.cinvcode = inventory.cinvcode
        join computationunit  with (nolock)  on computationunit.ccomunitcode = inventory.ccomunitcode
        left join 
            (select  SourceDetailId ,
                    max(PrinteDate) PrinteDate
            from    dbo.Addins_WHLabel  with (nolock) 
            where   LabelType = '库存标签'
            group by SourceDetailId )S1 on c.AutoID=s1.SourceDetailId
where   1=1
order by 
        c.cInvCode ,
        cBatch 
...
";
            sql = string.Format(sql, sourceFormCode, sourceDetailId, qty, "库存标签", invCode, invName, invStd, 
