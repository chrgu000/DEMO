...
{
    public partial class 高开返利单_SZ : UserControl
    {
        ...
        {
            string sSQL = @"
select cast(0 as bit) as choose,a.*,case when b.[PO项目] = '直营' then '直营' else '' end  as 是否直营,b.[金税发票号码]
from [_高开返利单_SZ] a left join [dbo].[_发票_sap] b on a.FPIDs = b.iID
where a.cCode = '{0}'
";
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                txt备注.Text = dt.Rows[0]["Remark"].ToString().Trim();
...
from _TH_GET_FLD_SZ 
where 1=1
	and FPIDs not in (select [FPIDs] from [dbo].[_高开返利单_SZ])
order by FPIDs
";
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                sSQL = @"
select * from _TH_GET_FHPrice_SZ
where ccuscode = '{0}' and cinvaddcode = '{1}'
...
";
                sSQL = string.Format(sSQL, sDLS, sInvCode, dQty, dDate.AddDays(1));
...
";
                    ...
                    sSQL = @"
update [dbo].[_高开返利单_SZ] set [auditUid] = null,[dtmAudit] = null
where [cCode] = '{0}'
";
                    sSQL = string.Format(sSQL, txt单据号.Text.Trim());
                    iCou = DbHelperSQL.ExecuteNonQuery(tran, CommandType.Text, sSQL);
