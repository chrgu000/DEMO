...
";
        ...
        private void SetEnable(bool b)
        {
            ...
            gridView_发票.OptionsBehavior.Editable = b;
...
select * 
from _TH_GET_FLD_HX 
where 1=1 and isnull(unmoney,0) <> 0 
order by iID
";
...
";
            ...
            if (lookUpEditcCusCode.Text.Trim() != "")
            {
                sSQL = sSQL.Replace("1=1", "1=1 and 代理商编码 = '" + lookUpEditcCusCode.EditValue.ToString().Trim() + "'");
            }
...
";
        ...
        private void txt核销金额_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                decimal d核销金额 = BaseFunction.ReturnDecimal(txt核销金额.Text.Trim());
                decimal d核销金额Temp =d核销金额;
                if (d核销金额 <= 0)
                {
                    throw new Exception("请输入核销金额");
                }
                ...
                for (int i = 0; i < gridView返利单.RowCount; i++)
                {
                    ...
                    {
                        gridView返利单.SetRowCellValue(i, gridColHXMoney, d未核销金额);
                        d核销金额Temp = d核销金额Temp - d未核销金额;
                    }
                    else
                    {
                        gridView返利单.SetRowCellValue(i, gridColHXMoney, d核销金额Temp);
                        d核销金额Temp = 0;
                    }
...
//------
                ...
                for (int i = 0; i < gridView返利单.RowCount; i++)
                {
                    ...
                    {
                        gridView_发票.SetRowCellValue(i, gridCol_HXMoney, d核销金额Temp);
                        d核销金额Temp = 0;
                    }
                    if (d未核销金额 == 0)
                        break;
