...
namespace 系统服务
{
    public static class 序号
    {
        ...
        {
            string sSQL = "select * from SerialNumber where TableID='" + TableName + "'";
        ...
        {
           string sSQL = "select * from SerialNumber where TableID='" + TableName + "' and Code='" + FieldName + "'";
            DataTable dt = clsSQLCommond.ExecQuery(sSQL);
        ...
        private static void GetTreeNode(DevExpress.XtraTreeList.TreeList treeList1, DevExpress.XtraTreeList.Nodes.TreeListNode tn, DataTable dt, string Code, string Name, string[] seq, int lev, int len)
        {
            if (tn.Tag.ToString().Trim() != "")
            {
                ...
                for (int i = 0; i < dw.Length; i++)
                {
                    ...
                    //if (dw[i][Code].ToString().Trim().Length == 2)
                    //{
                    //    tn1.Expanded = true;
                    //}
                }
        ...
        public static string GetNewSerialNumberContinuous(string TableName, string FieldName)
        {
            ...
            string sSQL = "select * from SerialNumberContinuous where TableID='" + TableName + "' and Code='" + FieldName + "'";
            DataTable dts = clsSQLCommond.ExecQuery(sSQL);
            string left = "";
            string middle = "";
            string right = "";
