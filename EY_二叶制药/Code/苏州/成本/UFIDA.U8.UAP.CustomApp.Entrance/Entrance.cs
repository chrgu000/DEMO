...
namespace UFIDA.U8.UAP.CustomApp.Entrance
{
    public class Entrance : NetLoginable
    {
        public override object CallFunction(string cMenuId, string cMenuName, string cAuthId, string cCmdLine)
        {
            ...
            if (sMenuID.Trim().ToLower() == "ProMaterial".ToLower())
            {
                //根据菜单读取自定义控件
                INetUserControl mycontrol = new ProMaterialCreater();

                base.ShowEmbedControl(mycontrol, cMenuId, true);
            }

            if (sMenuID.Trim().ToLower() == "车间材料领用汇总月报".ToLower())
            {
                //根据菜单读取自定义控件
                INetUserControl mycontrol = new 车间材料领用汇总月报Creater();

                base.ShowEmbedControl(mycontrol, cMenuId, true);
            }

            if (sMenuID.Trim().ToLower() == "成本分配表".ToLower())
            {
                //根据菜单读取自定义控件
                INetUserControl mycontrol = new 成本分配表Creater();

                base.ShowEmbedControl(mycontrol, cMenuId, true);
            }

            if (sMenuID.Trim().ToLower() == "费用分配表".ToLower())
            {
                //根据菜单读取自定义控件
                INetUserControl mycontrol = new 费用分配表Creater();

                base.ShowEmbedControl(mycontrol, cMenuId, true);
            }

            if (sMenuID.Trim().ToLower() == "能源分配".ToLower())
            {
                //根据菜单读取自定义控件
                INetUserControl mycontrol = new 能源分配Creater();

                base.ShowEmbedControl(mycontrol, cMenuId, true);
            }

            if (sMenuID.Trim().ToLower() == "辅助费用分配".ToLower())
            {
                //根据菜单读取自定义控件
                INetUserControl mycontrol = new 辅助费用分配Creater();
