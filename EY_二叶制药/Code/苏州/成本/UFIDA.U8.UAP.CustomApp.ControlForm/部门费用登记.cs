...
{
    public partial class 部门费用登记 : UserControl
    {
        ...
        private void SetLookUp()
        {
           ...
           DataTable dt = DbHelperSQL.Query(sSQL);
            ...
            sSQL = "select sType from [dbo].[_CostType] where iType = 2 order by sType";
        ...
        private void btnDel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                try
                {
                    ...
                    sSQL = "delete _部门费用登记 where  会计期间 = '111111' and 费用类型 = '" + s费用类型 + "'";
                    sSQL = sSQL.Replace("111111", s会计期间);
        ...
        private void GetGrid()
        {
            try
            {
                ...
                string sSQL = @"
select b.费用类型, b.会计期间,a.cDepCode as 部门编码,a.cDepName as 部门, b.金额, b.备注, b.制单人, b.制单日期, b.审核人, b.审核日期,b.修改人,b.修改日期,b.记账人,b.记账日期
from dbo.Department  a left join dbo._部门费用登记 b on a.cDepCode = b.部门 and b.会计期间 = '111111'  and b.费用类型 = '222222'
where isnull(a.bDepEnd,0) = 1 and isnull(a.cDepType,'') = '成本'
order by b.费用类型
";

                sSQL = sSQL.Replace("111111", lookUpEdit会计期间.EditValue.ToString());
                sSQL = sSQL.Replace("222222", lookUpEditCostType.EditValue.ToString());
        ...
        private void btnAudit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                try
                {
                    ...
                    string sSQL = "select count(1) from _部门费用登记 where 会计期间 = '111111' and 费用类型 = '" + s费用类型 + "'";
                    sSQL = sSQL.Replace("111111", s会计期间);
                    int iCount = BaseFunction.ReturnInt(DbHelperSQL.ExecuteDataset(tran, CommandType.Text, sSQL).Tables[0].Rows[0][0]);
                    if (iCount == 0)
                    ...
                    sSQL = "update _部门费用登记 set 审核人 = '" + sUserName + "',审核日期 = getdate() where  会计期间 = '111111' and 费用类型 = '" + s费用类型 + "'";
        ...
        private void lookUpEdi_EditValueChanged(object sender, EventArgs e)
        {
            try
            {
                if (lookUpEditCostType.EditValue == null || lookUpEditCostType.EditValue.ToString().Trim() == "")
