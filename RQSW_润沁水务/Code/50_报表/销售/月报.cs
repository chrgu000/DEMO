...
namespace 报表
{
    public static class 月报
    {
        ...
        static string s年;
        static string s统计方式;
        static string tablename = "StatByMonth";
        static DataTable dt出货;
        static DataTable dt出差费;
        public static DataTable Get(string s_部门, string s_业务员,string s_年, string s_月,string s_统计方式,bool issave)
        {
            ...
            try
            {
                ...
                if (s_统计方式 == "1")//按部门统计
                {
                    sSQL = "select S1 as cDepCode from SAPlan where S1 like '03%' and S1<>'03' and I1=2 and I2='" + s_月 + "' and s5 = '" + s_年 + "' ";
                    if (s_部门 != "")
                    {
                        sSQL = sSQL + " and S1='" + s_部门 + "'";
                    }
                }
                else//按人员统计
                {
                    sSQL = "select S1 as cDepCode,S3 as PersonCode from SAPlan where 1=1 and I1=2 and I2='" + s_月 + "' and s5 = '" + s_年 + "' ";
        ...
        public static string Get保底指标是否达成(string s月,string s年)
        {
            //s月 = s月;
            string sSQL = "";


            sSQL = "select S1 as cDepCode,S3 as PersonCode from SAPlan where 1=1 and I1=2 and I2='" + s月 + "' and s5 = '" + s年 + "' ";
            ...
            {
                if (dt.Rows[8][j].ToString() == "达成")
                {
                    if (emp != "")
                    {
                        emp = emp + ",";
                    }
                    emp = emp + dt.Columns[j].ColumnName;
                }
            }
            return emp;
        }
        ...
        {
            DataTable dts = 计划.月计划出货(s年, s月);

            if (dts.Rows.Count > 0)
            {
                Date1 = DateTime.Parse(dts.Rows[0]["Date1"].ToString()).ToString("yyyy-MM-dd");
                Date2 = DateTime.Parse(dts.Rows[0]["Date2"].ToString()).ToString("yyyy-MM-dd");
            }
...
sum(cast(D4 as decimal(18, 2))) as D4 from OperationalCosts a left join OperationalCostsDetails b on a.ID=b.ID
 where dDate>='" + Date1 + "' and dDate<='" + Date2 + "'  group by SS2,SS1) a left join (select S3,sum(D4) as 出差费用 from Mileage where I2='" + s月 + "' group by S3) b on a.SS1=b.S3";
        ...
        private static void GetRow业务招待费用(DataTable dt)
        {
            ...
            if (1 == 1)
            {
                ...
                for (int i = 1; i < dt.Columns.Count - 1; i++)
                {
                    if (s统计方式 == "1")
                    {
                        dw[i] = 系统服务.规格化.ReturnDecimal(dt出货.Compute("sum(业务招待费)", "cDepCode='" + dt.Columns[i].ColumnName + "' and Flag='1'"), 2);
                        sum = sum + 系统服务.规格化.ReturnDecimal(dw[i], 2);
                    }
                    else
                    {
                        dw[i] = 系统服务.规格化.ReturnDecimal(dt出货.Compute("sum(业务招待费)", "cPersonCode='" + dt.Columns[i].ColumnName + "' and Flag='1'"), 2);
                        sum = sum + 系统服务.规格化.ReturnDecimal(dw[i]);
                    }
                }
            ...
            if (1 == 1)
            {
                ...
                for (int i = 1; i < dt.Columns.Count - 1; i++)
                {
                    if (s统计方式 == "1")
                    {
                        dw[i] = 系统服务.规格化.ReturnDecimal(dt出货.Compute("sum(业务招待费)", "cDepCode='" + dt.Columns[i].ColumnName + "' and Flag='2'"), 2);
                        sum = sum + 系统服务.规格化.ReturnDecimal(dw[i], 2);
                    }
                    else
                    {
                        dw[i] = 系统服务.规格化.ReturnDecimal(dt出货.Compute("sum(业务招待费)", "cPersonCode='" + dt.Columns[i].ColumnName + "' and Flag='2'"), 2);
                        sum = sum + 系统服务.规格化.ReturnDecimal(dw[i]);
                    }
                }
            ...
            if (1 == 1)
            {
                ...
                for (int i = 1; i < dt.Columns.Count - 1; i++)
                {
                    dw[i] = 系统服务.规格化.ReturnDecimal(dt.Select("统计项='112'")[0][i]) - 系统服务.规格化.ReturnDecimal(dt.Select("统计项='111'")[0][i]) - -系统服务.规格化.ReturnDecimal(dt.Select("统计项='114'")[0][i]);
                    sum = sum + 系统服务.规格化.ReturnDecimal(dt.Select("统计项='112'")[0][i]) - 系统服务.规格化.ReturnDecimal(dt.Select("统计项='111'")[0][i]) - -系统服务.规格化.ReturnDecimal(dt.Select("统计项='114'")[0][i]);
                }
                dw["合计"] = sum;
                dt.Rows.Add(dw);
            }
        ...
        private static void GetRow出货利润(DataTable dt)
        {
            if (1 == 1)
            {
                ...
                //DataTable dts = 出货.Table出货利润(Date1, Date2);

                for (int i = 1; i < dt.Columns.Count - 1; i++)
                {
                    if (s统计方式 == "1")
                    {
                        dw[i] = 系统服务.规格化.ReturnDecimal(dt出货.Compute("sum(利润)", "cDepCode='" + dt.Columns[i].ColumnName + "' and Flag='1'"));
                        sum = sum + 系统服务.规格化.ReturnDecimal(dt出货.Compute("sum(利润)", "cDepCode='" + dt.Columns[i].ColumnName + "' and Flag='1'"));
                    }
                    else
                    {
                        dw[i] = 系统服务.规格化.ReturnDecimal(dt出货.Compute("sum(利润)", "cPersonCode='" + dt.Columns[i].ColumnName + "' and Flag='1'"));
                        sum = sum + 系统服务.规格化.ReturnDecimal(dt出货.Compute("sum(利润)", "cPersonCode='" + dt.Columns[i].ColumnName + "' and Flag='1'"));
                    }
                }
            ...
            if (1 == 1)
            {
                ...
                for (int i = 1; i < dt.Columns.Count - 1; i++)
                {
                    if (s统计方式 == "1")
                    {
                        dw[i] = 系统服务.规格化.ReturnDecimal(dt出货.Compute("sum(利润)", "cDepCode='" + dt.Columns[i].ColumnName + "' and Flag='2'"));
                        sum = sum + 系统服务.规格化.ReturnDecimal(dt出货.Compute("sum(利润)", "cDepCode='" + dt.Columns[i].ColumnName + "' and Flag='2'"));
                    }
                    else
                    {
                        dw[i] = 系统服务.规格化.ReturnDecimal(dt出货.Compute("sum(利润)", "cPersonCode='" + dt.Columns[i].ColumnName + "' and Flag='2'"));
                        sum = sum + 系统服务.规格化.ReturnDecimal(dt出货.Compute("sum(利润)", "cPersonCode='" + dt.Columns[i].ColumnName + "' and Flag='2'"));
                    }
                }
        ...
        private static void GetRow累计出货数量(DataTable dt)
        {
            ...
            for (int i = 1; i < dt.Columns.Count - 1; i++)
            {
                if (s统计方式 == "1")
                {
                    dw[i] = 系统服务.规格化.ReturnDecimal(dts.Compute("sum(iQuantity)", "cDepCode='" + dt.Columns[i].ColumnName + "'"), 2);
                    sum = sum + 系统服务.规格化.ReturnDecimal(dw[i], 2);
                }
                else
                {
                    dw[i] = 系统服务.规格化.ReturnDecimal(dts.Compute("sum(iQuantity)", "cPersonCode='" + dt.Columns[i].ColumnName + "'"), 2);
                    sum = sum + 系统服务.规格化.ReturnDecimal(dw[i], 2);
                }
            }
        ...
        private static void GetSave(DataTable dt, DataTable dts)
        {
            ...
            sSQL = "delete from " + tablename + " where Month='" + s月 + "'";
            aList.Add(sSQL);
        ...
        private static void GetRow表头(DataTable dt)
        {
            ...
            sSQL = "select * from  _LookUpDate where iType=30";
