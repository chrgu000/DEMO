...
namespace 系统服务
{
    public static class 变更表
    {
        ...
        public static bool GetChange(string tablename, string tablenames, string tablenamel,string tablenamec, string id, ClsGetSQL clsGetSQL, System.Collections.ArrayList aList)
        {
            ...
            sSQL = "select isnull(max(HistoryId)+1,1) as HistoryId from ( "
                + "select isnull(max(HistoryId)+1,1) as HistoryId from " + tablename + "History "
                + "union all select isnull(max(HistoryId)+1,1) as HistoryId from " + tablenames + "History "
                + "union all select isnull(max(HistoryId)+1,1) as HistoryId from " + tablenamel + "History "
                + "union all select isnull(max(HistoryId)+1,1) as HistoryId from " + tablenamec + "History ) a ";
            long HistoryId = long.Parse(clsSQLCommond.ExecQuery(sSQL).Rows[0][0].ToString());
            long HistoryIds = HistoryId;
            long HistoryIdsl = HistoryId;
            long HistoryIdsc = HistoryId;
