...
namespace FrameBaseFunction
{
    public class ClsGetSQL
    {
        ...
        {
            if (sDataBaseName.ToLower().Trim() == "systemdb_th")
                sDataBaseName = FrameBaseFunction.ClsBaseDataInfo.sDataBaseName;
        ...
        {
            return GetDeleteSQL(FrameBaseFunction.ClsBaseDataInfo.sDataBaseName, sTableName, dtTemp, iRow);
        }
        ...
        public string GetDeleteSQL(string sDataBaseName, string sTableName, DataTable dtTemp, int iRow)
        {
            ...
            string sDBTemp = "SystemDB_TH";
            if (sDataBaseName.Length > 8 && sDataBaseName.Trim().Substring(0, 9).ToLower() != "systemdb_")
            {
                sDBTemp = sDataBaseName;
            }

            string sSQL = "select * from _TableColInfo " +
                            "WHERE TABLE_NAME = '" + sTableName + "' AND TABLE_CATALOG= '" + sDBTemp + "' " +
                            "order by iID";
        ...
        {
            return GetUpdateSQL(FrameBaseFunction.ClsBaseDataInfo.sDataBaseName, sTableName, dtTemp, iRow);
        }
        ...
        {
            return GetInsertSQL(FrameBaseFunction.ClsBaseDataInfo.sDataBaseName, sTableName, dtTemp, iRow);
        }
