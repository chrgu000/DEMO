...
namespace UFIDA.U8.UAP.CustomApp.ControlForm
{
    public partial class Sa_BOMAll : UserControl
    {
        ...
        {
            object[] obj = AddNodeObj(tl);
            TreeListNode node = treeList1.AppendNode(obj, -1);
            //如果修改后的库存金额小于0再展开一层
            if (规格化.ReturnDecimal(obj[14],2) < 0)
            {
                DataRow[] dwr = dtt.Select("母件编码='" + tl + "' and 子件物料属性='自制件' ");
                for (int i = 0; i < dwr.Length; i++)
                {
                    string tlzj = dwr[i]["子件编码"].ToString();
                    TreeListNode node1 = treeList1.AppendNode(AddNodeObj(tlzj), node);
                }
            }
        }

        private object[] AddNodeObj(string tl)
        {
            DataRow[] dwc = dtc.Select("cInvCode='" + tl + "'");
            string cCusInvCode = "";
            string cCusName = "";
            ...
            //标准成本
            decimal d标准成本单价 = 0;
            DataRow[] dwcb = dtcb.Select("存货编码='" + tl + "'");
            if (dwcb.Length > 0)
            {
                d标准成本单价 = 规格化.ReturnDecimal(dwcb[0]["标准成本合计"], 4);
            }
...
CONVERT(nvarchar(40), CASE vp.InvAttr WHEN 1 THEN '采购件' WHEN 2 THEN '委外件' WHEN 3 THEN '自制件' WHEN 4 THEN '选项类' WHEN 5 THEN 'PTO' WHEN 6 THEN '' WHEN 7 THEN '计划品' ELSE '' END) AS 母件物料属性,
 ...
 ( CASE o.WIPType WHEN 2 THEN '工序倒冲' WHEN 1 THEN '入库倒冲' WHEN 3 THEN '领用' WHEN 4 THEN '虚拟件' ELSE '直接供应' END) AS 供应类型 
        ...
        {
            string sSQL = @"select distinct cInvCode from CusInvContrapose";
            return SqlHelper.ExecuteDataset(Conn, CommandType.Text, sSQL).Tables[0];
        } 
        ...
        {
            string sSQL = @"select cInvCode,iQuantity,iTaxUnitPrice,ddate  from SO_SOMain  a left join SO_SODetails b on a.ID =b.ID 
                where ddate>='" + DateTime.Parse(ed).AddDays(-14).ToString("yyyy-MM-dd") + "' and ddate<='" + ed + "' and isnull(cVerifier,'')<>'' ";
            return SqlHelper.ExecuteDataset(Conn, CommandType.Text, sSQL).Tables[0];
        }
        ...
        {
            string sSQL = @"if object_id(N'_temp_chdwbzcb',N'U') is not null drop table _temp_chdwbzcb
declare @p1 int
set @p1=0
exec sp_prepexecrpc @p1 output,N'Sp_Co_InvUnitStdCost',4,' 1=1 ','1<>1','_temp_chdwbzcb',0
select @p1";
            SqlHelper.ExecuteScalar(Conn, CommandType.Text, sSQL);
            sSQL = "select * from _temp_chdwbzcb";
            return SqlHelper.ExecuteDataset(Conn, CommandType.Text, sSQL).Tables[0];
        }
...
select @p1";
            ...
            SqlHelper.ExecuteScalar(Conn, CommandType.Text, sSQL);
            sSQL = "select *,ISNULL(实际材料费用,0)+ISNULL(实际人工费用,0)+ISNULL(实际固定制造费用,0)+ISNULL(实际变动制造费用,0)+ISNULL(实际委外加工费,0) as 结存金额 from _temp_sfchz";
            return SqlHelper.ExecuteDataset(Conn, CommandType.Text, sSQL).Tables[0];
        }
