...
from SO_SOMain a inner join SO_SODetails b on a.ID = b.ID
    ...
    inner join Inventory_Sub e on e.cInvSubCode = d.cInvCode
...
";
                        ...
                        if (dtSO.Rows[0]["cPayCode"].ToString().Trim() != "")
                        {
                        ...
                        model.cDefine10 = dtSO.Rows[0]["cDefine10"].ToString().Trim();
                        ...
                        for (int j = 0; j < gridView1.RowCount; j++)
                        {
                            ...
                            string sWhCode = gridView1.GetRowCellValue(i, gridCol仓库编码).ToString().Trim();


                            DataRow[] drs = dtSO.Select("cInvCode = '" + sInvCode.ToString() + "'");
                            if (drs.Length != 1)
                            {
                                sErr = sErr + "销售订单行号 " + s销售订单号 + " 存货编码不存在或同一订单有两行相同存货编码\n";
                                continue;
                            }

                            decimal dFHSX = BaseFunction.ReturnDecimal(drs[0]["fInvOutUpLimit"]);
                            decimal dSOQty = BaseFunction.ReturnDecimal(drs[0]["iQuantity"]);
                            decimal diFHQuantity = BaseFunction.ReturnDecimal(drs[0]["iFHQuantity"]);
                            decimal dQty =  BaseFunction.ReturnDecimal(gridView1.GetRowCellValue(j, gridCol数量));
                            if (dSOQty * (1+ dFHSX/100) < diFHQuantity + dQty)
                            {
                                sErr = sErr + "行 " + (j + 1).ToString() + " 超订单发货\n";
                                continue;
                            }
                            
                            sSQL = @"
select sum(isnull(iQuantity,0) - isnull(fOutQuantity,0)) as iQty,cInvCode
from CurrentStock 
WHERE   (cInvCode = 'aaaaaaaa') AND (cWhCode = 'bbbbbbbb')
group by cInvCode
";
                            ...
                            models.cWhCode = sWhCode;
                            models.cInvCode = drs[0]["cInvCode"].ToString();
                            ...
                            //models.iNum = 
                            models.iTaxRate = BaseFunction.ReturnDecimal(drs[0]["iTaxRate"]);
                            models.iQuotedPrice = 0;
                            models.iTaxUnitPrice = BaseFunction.ReturnDecimal(drs[0]["iTaxUnitPrice"]);
                            models.iUnitPrice = BaseFunction.ReturnDecimal(drs[0]["iUnitPrice"]);
                            models.imoneysum = 0;
                            models.iMoney = BaseFunction.ReturnDecimal(models.iUnitPrice * dQty, 2);
                            models.iSum = BaseFunction.ReturnDecimal(models.iTaxUnitPrice * dQty, 2);
                            models.iTax = models.iSum - models.iMoney;

                            models.iNatUnitPrice = BaseFunction.ReturnDecimal(drs[0]["iNatUnitPrice"]);
                            models.iNatMoney = BaseFunction.ReturnDecimal(models.iNatUnitPrice * dQty, 2);
                            models.iNatSum = BaseFunction.ReturnDecimal(models.iNatUnitPrice * (1 + models.iTaxRate / 100) * dQty, 2);
                            ...
                            models.iTB = 0;
                            models.TBQuantity = 0;
                            models.iSOsID = BaseFunction.ReturnLong(drs[0]["iSOsID"]);
                            models.iDLsID = lIDDetails;
                            models.KL = 100;
                            models.KL2 = 100;
                            models.cInvName = drs[0]["cInvName"].ToString().Trim();
                            ...
                            models.iorderrowno = BaseFunction.ReturnLong(drs[0]["iRowNo"]);
                            ...
                            models.cDefine31 = drs[0]["cDefine31"].ToString().Trim();
...
";
                            sSQL = sSQL.Replace("aaaaaa", drs[0]["iSOsID"].ToString().Trim());
                            DbHelperSQL.ExecuteNonQuery(tran, CommandType.Text, sSQL);
...
if exists
    ...
    begin 
        ...
        if( @iCount > 0 )
            ...
            insert into CurrentStock(cWhCode,cInvCode,fOutQuantity,itemid, cFree1, cFree2, cFree3, cFree4, cFree5, cFree6, cFree7, cFree8, cFree9, cFree10,cBatch,iSoDid)
...
";
                            ...
                            sSQL = sSQL.Replace("@cWhCode", models.cWhCode);
