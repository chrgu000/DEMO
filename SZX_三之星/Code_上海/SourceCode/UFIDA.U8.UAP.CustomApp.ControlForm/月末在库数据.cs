...
{
    public partial class 月末在库数据 : UserControl
    {
        ...
        private void SetLookUp()
        {
            ...
            //sSQL = "select cWhCode,cWhName from WareHouse order by cWhCode"; dt = DbHelperSQL.Query(sSQL);
            //dr = dt.NewRow();
            //dt.Rows.InsertAt(dr, 0);
            //lookUpEditcWhCode1.Properties.DataSource = dt;
            //lookUpEditcWhCode2.Properties.DataSource = dt;
        }
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                sF.DefaultExt = "csv";
                sF.FileName = "bilis_Monthly_Stock_Master.csv";
...
SELECT cast(1 as bit) as 选择
        ...
        ,getdate() as 在库年月
        ,cast(isnull(g.iUnitPrice,0) as decimal(16,2)) as [成本合计(单价)]
FROM TH_Curr_Qty A INNER JOIN dbo.Inventory B ON A.CINVCODE = B.cInvCode
    ...
    LEFT JOIN (
		    SELECT a.cInvCode
		    ,CASE WHEN iMoney/NULLIF(iNum,0) IS null THEN iOMoney/NULLIF(iONum ,0)
		    ELSE iMoney/NULLIF(iNum,0) END AS iUnitPrice 
		FROM IA_Summary a
			INNER JOIN 
			(
				SELECT cInvCode,MAX(AutoID) AS AutoID 
				FROM IA_Summary 
				-- WHERE ISNULL(iUnitPrice,0) <> 0  2016-12-14 沈平修改
				WHERE (ISNULL(iMoney/NULLIF(iNum,0) ,0) <> 0 OR ISNULL(iOMoney/NULLIF(iONum ,0),0)<>0)
				
				GROUP BY cInvCode
			)b ON a.AutoID = b.Autoid
	 ) g ON g.cInvCode=b.cInvCode
