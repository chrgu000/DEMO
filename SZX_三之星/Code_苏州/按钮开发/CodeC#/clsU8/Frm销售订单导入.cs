...
";
        ...
        private void btnLoad_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string sSQL = "select *,0 as 价格表单价,原币含税单价 as 原币含税单价,存货编码 as 存货编码,存货编码 as 存货编码2 from [Sheet1$]";
                ...
                {
                    if (dt.Rows[i]["客户订单号"].ToString().Trim() == "" && dt.Rows[i]["客户编号"].ToString().Trim() == "")
                    {
                        dt.Rows.RemoveAt(i);
                    }
                }
                ...
                for (int i = 0; i < dt.Columns.Count; i++)
                {
                    ...
                    string sColName2 = sJapToChina(sColName);
                    dt.Columns[i].ColumnName = sColName2;
                    dt.Columns[i].Caption = sColName2;
                }
...
";
                ...
                {
                    dt.Rows[i]["存货编码"] = dt.Rows[i]["存货编码"].ToString().Trim();
                    #region 存货与客户存货

                    sSQL = @"
select a.cInvCode 
from Inventory a left join CusInvContrapose b on a.cInvCode = b.cInvCode and b.cCusCode = 'aaaaaa'
where a.cInvCode = N'bbbbbb' or b.cCusInvCode = N'bbbbbb'
";
                    sSQL = sSQL.Replace("aaaaaa", dt.Rows[i]["客户编号"].ToString().Trim());
                    sSQL = sSQL.Replace("bbbbbb", dt.Rows[i]["存货编码"].ToString().Trim());
                    DataTable dtInvCus = DbHelperSQL.ExecuteDataset(sConnString, CommandType.Text, sSQL).Tables[0];

                    if (dtInvCus != null && dtInvCus.Rows.Count > 0)
                    {
                        dt.Rows[i]["存货编码"] = dtInvCus.Rows[0]["cInvCode"].ToString().Trim();
                    }
                    ...
                    DateTime d单据日期 = dateEdit1.DateTime;
                    dt.Rows[i]["单据日期"] = d单据日期.ToString("yyyy-MM-dd");
                    dt.Rows[i]["预完工日期"] = d单据日期.ToString("yyyy-MM-dd");
                    dt.Rows[i]["预发货日期"] = d单据日期.ToString("yyyy-MM-dd");

                    string sCusCode = dt.Rows[i]["客户编号"].ToString().Trim();
...
";
                    ...
                    for (int i = 0; i < gridView1.RowCount; i++)
                    {
                        ...
                        string s客户订单号 = gridView1.GetRowCellValue(i, gridCol客户订单号).ToString().Trim();
...
";
                        ...
                        string sCusCode = gridView1.GetRowCellValue(i, gridCol客户编号).ToString().Trim();
                        if (sCusCode == "")
                        {
                        ...
                        //model.cCusOAddress;
                        //model.cPayCode;
                        string sCexch_Code = gridView1.GetRowCellValue(i, gridCol币种).ToString().Trim();
                        ...
                        string s业务类型 = gridView1.GetRowCellValue(i, gridCol业务类型).ToString().Trim();
                        ...
                        {
                            if (s客户订单号 != gridView1.GetRowCellValue(j, gridCol客户订单号).ToString().Trim())
                            {
                                continue;
                            }
...
";
                            ...
                            sSQL = sSQL.Replace("222222", gridView1.GetRowCellValue(j, gridCol存货编码).ToString().Trim());
                            ...
                            DateTime d预发货日期 = BaseFunction.ReturnDate(gridView1.GetRowCellValue(j, gridCol单据日期));
                            ...
                            models.iQuantity = BaseFunction.ReturnDecimal(gridView1.GetRowCellValue(j, gridView1.Columns["数量"]).ToString().Trim());
                            ...
                            decimal d原币含税单价 = BaseFunction.ReturnDecimal(gridView1.GetRowCellValue(j, gridCol原币含税单价));
                            if (d原币含税单价 == 0)
                            {
                            ...
                            decimal d数量 = BaseFunction.ReturnDecimal(gridView1.GetRowCellValue(j, gridCol数量));


                            decimal d原币含税金额 = BaseFunction.ReturnDecimal(d原币含税单价 * d数量, 3);
                            decimal d原币无税单价 = BaseFunction.ReturnDecimal(d原币含税单价 / (1 + BaseFunction.ReturnDecimal(gridView1.GetRowCellValue(i, gridCol税率) )/ 100), 6);
                            ...
                            decimal d本币无税单价 = BaseFunction.ReturnDecimal(d本币含税单价 /  (1 + BaseFunction.ReturnDecimal(gridView1.GetRowCellValue(i, gridCol税率)) / 100), 6);
                            ...
                            DateTime d预完工日期 = BaseFunction.ReturnDate(gridView1.GetRowCellValue(j, gridCol单据日期));
                            ...
                            modelsE.cbdefine2 = BaseFunction.ReturnDate(gridView1.GetRowCellValue(j, gridCol单据日期));
...
//                    DbHelperSQL.ExecuteNonQuery(tran, CommandType.Text, sSQL);
        ...
        private void gridView1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string s销售订单号 = gridView1.GetRowCellValue(gridView1.FocusedRowHandle, gridCol销售订单号).ToString().Trim();

                for (int i = 0; i<gridView1.RowCount; i++)
                {
                    if (gridView1.GetRowCellValue(i, gridCol销售订单号).ToString().Trim() == s销售订单号)
                    {
