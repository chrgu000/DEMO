...
";
        ...
        private void GetGrid()
        {
            ...
            sSQL = @"
select b.cInvCode as 存货编码,c.cInvName as 存货名称,c.cInvStd as 规格型号,cast(sum(b.iQuantity) as decimal(16,3)) as 数量,d.cidefine12 as 原价
	,cast(cast(sum(b.iQuantity) as decimal(16,3)) * d.cidefine12 as decimal(16,6)) as 标准
	,cast(null as decimal(16,6)) as 差异
	,cast(null as decimal(16,6)) as 间接原价
	,cast(null as decimal(16,6)) as 实际成本
from rdrecord10 a inner join rdrecords10 b on a.id = b.ID
	inner join Inventory c on b.cInvCode = c.cInvCode
	left join 
	(
        select *
		from _sosp_Inventory_StCost
		 where iYear = case when month('aaaaaa') < 4 then year('aaaaaa')-1 else year('aaaaaa') end 
    )d on c.cInvCode = d.cInvCode 
where 1=1
...
";
            ...
            sSQL = sSQL.Replace("aaaaaa", d.ToString("yyyy-MM-dd"));
