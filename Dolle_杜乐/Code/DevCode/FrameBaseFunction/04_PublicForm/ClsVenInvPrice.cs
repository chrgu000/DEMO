...
namespace FrameBaseFunction
{
    public class ClsVenInvPrice
    {
        ...
        public DataTable GetPrice(int isupplytype)
        {
            ...
            //                "( " +
            //                "select max(Autoid) as Autoid from @u8.ven_inv_price where iSupplyType =  '" + isupplytype + "' " + "group by cvencode,cInvCode " +
...
";
 ...
 from ( 
			...
			select max(dEnableDate) as dEnableDate,cvencode,cInvCode  
			from @u8.ven_inv_price 
            where iSupplyType =  '{0}' and cInvCode = '{1}'
			group by cvencode,cInvCode 
...
";
            ...
            sSQL = string.Format(sSQL, isupplytype, cInvCode);
            return clsSQL.ExecQuery(sSQL);
        }
    }
}
