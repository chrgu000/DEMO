...
namespace TH.BaseClass
{
        ...
        public static ClsDataBase Instance()
        {
            if (clsCommond == null)
            {
                lock (lockHelper)
                {
                    if (clsCommond == null)
                    {
                        //if (TH.BaseClass.ClsBaseDataInfo.sDataBaseType.ToLower().Trim() == "sqlserver")
                            clsCommond = new TH.BaseClass.ClsSQLServerCommond();
                        //if (TH.BaseClass.ClsBaseDataInfo.sDataBaseType.ToLower().Trim() == "webservice")
                        //    clsCommond = new TH.BaseClass.ClsWebServerice();
                    }
                }
            }
            return clsCommond;
    ...
    /// 数据访问基础类(基于SQLServer)
    /// </summary>
    public class ClsSQLServerCommond : TH.BaseClass.ClsDataBase
    {
        ...
        {
            using (SqlConnection connection = new SqlConnection(TH.BaseClass.ClsBaseDataInfo.sConnString))
            {
                using (SqlCommand cmd = new SqlCommand(SQLString, connection))
                {
                    try
                    {
                        SQLString = SQLString.Replace("@u8", TH.BaseClass.ClsBaseDataInfo.sUFDataBaseName.Trim() + ".dbo");
        ...
        //        sDataBaseName = TH.BaseClass.ClsBaseDataInfo.sUFDataBaseName;
        ...
        {
            int iExecCou = 0;
            using (SqlConnection conn = new SqlConnection(TH.BaseClass.ClsBaseDataInfo.sConnString))
            {
                conn.Open();
                ...
                try
                {
                    ...
                    {
                        strsql = SQLStringList[n].ToString().Replace("@u8", TH.BaseClass.ClsBaseDataInfo.sUFDataBaseName.Trim() + ".dbo");
    ...
    //public class ClsWebServerice : TH.BaseClass.ClsDataBase
    //{
    //    ClsDES clsDES = ClsDES.Instance();

    //    WebReference.DBWebService DBService = new TH.BaseClass.WebReference.DBWebService();

    //    public ClsWebServerice()
    //    {
    //        DBService.Url = TH.BaseClass.ClsBaseDataInfo.sWebURL;
    //    }
    ...
    //            try
    //            {
    //                strsql = SQLString.Replace("@u8", TH.BaseClass.ClsBaseDataInfo.sUFDataBaseName.Trim() + ".dbo");
    //                if (strsql.Trim().Length > 1)
    //                {
    //                    cmd.CommandText = strsql;
    //                    iExecCou = iExecCou + cmd.ExecuteNonQuery();
    ...
    //                {
    //                    s = s + SQLStringList[i].ToString().Replace("@u8", TH.BaseClass.ClsBaseDataInfo.sUFDataBaseName.Trim() + ".dbo").Trim() + "ヵ";
    //                }
