...
convert(decimal (38,6),IsNull(Inventory.iInvSPrice,0)) as iUnitCost,  CU_F.cComUnitName as cInvA_Unit ,  (case when  ( dDate>=N444444 and isnull(bisstqc,0)=0) then 1 else 0 end) as bBQFS , 
(case when  ( dDate>=N444444 and isnull(bisstqc,0)=0) then case when brdflag=0 then 1 else 0 end  else 0 end) as bBQCK , 
(case when  ( dDate>=N444444 and isnull(bisstqc,0)=0) then case when brdflag=1 then 1 else 0 end else 0 end) as bBQRK , isnull(Left(RdRecord.cRdCode,5) ,N'')+ convert(nvarchar(1),brdflag)  as newrdflag
...
,convert(decimal(18, 3),null) as iQty10,convert(decimal(18, 3),null) as iQty11,convert(decimal(18, 3),null) as iQty32,convert(datetime,null)  dDate,convert(varchar(50),null) as Flag
INTO #temp1 from 
...
,convert(decimal(18, 3),null) as iQty10,convert(decimal(18, 3),null) as iQty11,convert(decimal(18, 3),null) as iQty32,dDate,'01' as Flag
from 
rdrecord01 rdrecord left join rdrecords01 rdrecords on rdrecord.id=rdrecords.id 
left join inventory on rdrecords.cinvcode=inventory.cinvcode 
left join warehouse on RdRecord.cWhCode = Warehouse.cWhCode
...
,convert(decimal(18, 3),null) as iQty10,convert(decimal(18, 3),null) as iQty11,convert(decimal(18, 3),null) as iQty32,dDate,'08' as Flag
from 
...
,convert(decimal(18, 3),null) as iQty10,convert(decimal(18, 3),null) as iQty11,convert(decimal(18, 3),null) as iQty32,dDate,'09' as Flag
from 
...
,convert(decimal(18, 3),null) as 累计其他入库,convert(decimal(18, 3),null) as 累计其他出库,cVouchType
,convert(decimal(18, 3),null) as iQty01,convert(decimal(18, 3),null) as iQty08,convert(decimal(18, 3),null) as iQty09
,convert(decimal(18, 3),i.iquantity) as iQty10,convert(decimal(18, 3),null) as iQty11,convert(decimal(18, 3),null) as iQty32,dDate,'10' as Flag
from 
...
,convert(decimal(18, 3),null) as 累计其他入库,convert(decimal(18, 3),null) as 累计其他出库,cVouchType
,convert(decimal(18, 3),null) as iQty01,convert(decimal(18, 3),null) as iQty08,convert(decimal(18, 3),null) as iQty09
,convert(decimal(18, 3),null) as iQty10,convert(decimal(18, 3),i.iquantity) as iQty11,convert(decimal(18, 3),null) as iQty32,dDate,'11' as Flag
from 
...
,convert(decimal(18, 3),null) as 累计其他入库,convert(decimal(18, 3),null) as 累计其他出库,cVouchType
,convert(decimal(18, 3),null) as iQty01,convert(decimal(18, 3),null) as iQty08,convert(decimal(18, 3),null) as iQty09
,convert(decimal(18, 3),null) as iQty10,convert(decimal(18, 3),null) as iQty11,convert(decimal(18, 3),i.iquantity) as iQty32,dDate,'32' as Flag
from 
...
,convert(decimal(18, 3),null) as 累计其他入库,convert(decimal(18, 3),null) as 累计其他出库,cVouchType
,convert(decimal(18, 3),null) as iQty01,convert(decimal(18, 3),null) as iQty08,convert(decimal(18, 3),null) as iQty09
,convert(decimal(18, 3),null) as iQty10,convert(decimal(18, 3),null) as iQty11,convert(decimal(18, 3),null) as iQty32,dDate,'34' as Flag
from 
...
insert into #temp1   Select autoid,warehouse.cwhcode,cWhName,inventory.cInvCCode ,cInvCName ,a.cinvcode,inventory.cInvStd,inventory.cInvName,IsNull(cItem_class,N'') AS cItem_class ,
...
0 as bBQCK , 
0 as bBQRK , isnull(Left(cRdCode,5) ,N'')+ convert(nvarchar(1),1)  as newrdflag 
,convert(decimal(18, 3),null) as 累计入库,convert(decimal(18, 3),null) as 累计领料,convert(decimal(18, 3),null) as 累计退库
,convert(decimal(18, 3),null) as 采购入库,convert(decimal(18, 3),null) as 退货数量
,convert(decimal(18, 3),null) as 累计销售,convert(decimal(18, 3),null) as 累计退货
,convert(decimal(18, 3),null) as 累计其他入库,convert(decimal(18, 3),null) as 累计其他出库,null
,convert(decimal(18, 3),null) as iQty01,convert(decimal(18, 3),null) as iQty08,convert(decimal(18, 3),null) as iQty09
,convert(decimal(18, 3),null) as iQty10,convert(decimal(18, 3),null) as iQty11,convert(decimal(18, 3),null) as iQty32,null as dDate,null as Flag
from 
...
where 3=3 and  iYear = " + DateTime.Parse(sdate).Year + " and iMonth = " + DateTime.Parse(sdate).Month + "  ";
            if (flag == 1)
            {
                sSQL = sSQL + @"Select a.cInvCCode ,ic1.cInvCCode as cInvCCode1,ic1.cInvCName as cInvCName1,ic2.cInvCCode as cInvCCode2,ic2.cInvCName as cInvCName2,ic3.cInvCCode as cInvCCode3,ic3.cInvCName as cInvCName3
...
,cDefine3,cDefine9,cDefine22,cDefine25,cDefine28,cDefine31,cDefine1,cDefine2,cDefine10,cDefine23,cDefine24,cDefine26
...
,sum(ISNULL(iInQuantity,0)-ISNULL(iOutQuantity,0)) as 本期变动,sum(dd.采购订单在途) as 采购订单在途
...
,sum(case when dDate>=222222 and dDate<=333333 then isnull(iQty01,0)+isnull(iQty08,0)+isnull(iQty09,0) end) as 本期出库
,sum(case when dDate>=222222 and dDate<=333333 then isnull(iQty10,0)+isnull(iQty11,0)+isnull(iQty32,0) end) as 本期入库
...
left join (select cInvCode,sum(isnull(iQuantity,0)-isnull(finquantity ,0)) as 采购订单在途 from SO_SODetails  group by cInvCode having sum(isnull(iQuantity,0)-isnull(finquantity ,0))>0) dd on a.cinvcode=dd.cinvcode
...
,cDefine3,cDefine9,cDefine22,cDefine25,cDefine28,cDefine31,cDefine1,cDefine2,cDefine10,cDefine23,cDefine24,cDefine26";
            ...
            {
                sSQL = sSQL + @"Select 
cWhCode as 仓库编码, cWhName as 仓库名称, a.cInvCode, a.cInvName,cInvAddCode,  a.cInvStd, cAddress AS 产地, ComputationUnit.CComUnitName  AS 计量单位,cInvA_Unit AS 库存单位, a.cInvCCode  , cItem_class AS 项目大类编码,cItemCName AS 项目大类名称 ,cItemCode AS 项目编码 ,cName AS 项目名称 ,
cFree1,cFree2,cFree3,cFree4,cInvDefine1,cInvDefine2,cInvDefine3,cInvDefine4,cInvDefine5,cInvDefine6,cInvDefine7,cDefine3,cDefine9,cDefine22,cDefine25,cDefine28,cDefine31,cDefine1,cDefine2,cDefine10,cDefine23,cDefine24,cDefine26,
sum(iQCJCSL) AS  期初结存数量  ,
sum(iInQuantity) as 总计入库数量,
sum(iOutQuantity) as 总计出库数量,
sum(iQMJCSL) AS 期末结存数量,cRdName,a.cRdCode
from #temp1 A  left join inventory inv on a.cinvcode=inv.cinvcode left join ComputationUnit ON inv.cComUnitCode = ComputationUnit.cComUnitCode 
left join Rd_Style rs on rs.cRdCode=a.cRdCode 
where 4=4 group by 
cWhCode , cWhName , a.cInvCode, a.cInvName,cInvAddCode,  a.cInvStd, cAddress , ComputationUnit.CComUnitName,cInvA_Unit, a.cInvCCode  , cItem_class ,cItemCName  ,cItemCode  ,cName  ,
cFree1,cFree2,cFree3,cFree4,cInvDefine1,cInvDefine2,cInvDefine3,cInvDefine4,cInvDefine5,cInvDefine6,cInvDefine7,cDefine3,cDefine9,cDefine22,cDefine25,cDefine28,cDefine31,cDefine1,cDefine2,cDefine10,cDefine23,cDefine24,cDefine26
,cRdName,a.cRdCode
";
                if (cRdCode != "")
...
sum(iOutQuantity) as 总计出库数量,sum(iOutNum) as  总计出库件数 ,  convert(decimal(36,2),sum(iOutQuantity*iUnitCost))  As 总计出库金额 ,
sum(iQMJCSL) AS 期末结存数量,sum(iQMJCJS) AS 期末结存件数,  convert(decimal(36,2),sum(iQMJCSL*iUnitCost))  AS 期末结存金额
,sum(ISNULL(iInQuantity,0)-ISNULL(iOutQuantity,0)) as 本期变动,sum(dd.采购订单在途) as 采购订单在途,convert(decimal(36,2),null) as 期初数量,convert(decimal(36,2),null) as 期末数量
from #temp1 A  left join inventory inv on a.cinvcode=inv.cinvcode left join ComputationUnit ON inv.cComUnitCode = ComputationUnit.cComUnitCode 
left join InventoryClass ic1 on ic1.cInvCCode = left(inv.cInvCCode,2)
left join InventoryClass ic2 on ic2.cInvCCode = left(inv.cInvCCode,4)
left join InventoryClass ic3 on ic3.cInvCCode = left(inv.cInvCCode,6) 
...
";
            ...
            if (bcHandler == true)
            {
                sSQL = sSQL.Replace("1=1", "1=1 and isnull(cHandler,'')<>''");
            }
            else
            {
                sSQL = sSQL.Replace("1=1", "1=1 and isnull(cHandler,'')=''");
            ...
            if (bcHas == false)
            {
                sSQL = sSQL+" having sum(ISNULL(iInQuantity,0)-ISNULL(iOutQuantity,0))<>0";
            }
            
            DataTable dt = SqlHelper.ExecuteDataset(Conn, CommandType.Text, sSQL).Tables[0];
            if (flag == 3)
            {
                decimal iQuantity = 系统服务.规格化.ReturnDecimal(dt.Compute("sum(期初结存数量)", ""), 3);
...
";
        ...
        {
            string sSQL = @"
select AutoID,cBatchProperty6,cBatchProperty7,cBatchProperty8,cBatchProperty9,cbMemo,b.cCode ,b.dDate,r.cRdName,'01' as Flag into #b from rdrecords01 a 
left join rdrecord01 b on a.ID=b.ID left join rd_style r on b.cRdCode =r.cRdCode 

insert into #b select AutoID,cBatchProperty6,cBatchProperty7,cBatchProperty8,cBatchProperty9,cbMemo,b.cCode ,b.dDate,r.cRdName,'08' as Flag from rdrecords08 a 
left join rdrecord08 b on a.ID=b.ID left join rd_style r on b.cRdCode =r.cRdCode 

insert into #b select AutoID,cBatchProperty6,cBatchProperty7,cBatchProperty8,cBatchProperty9,cbMemo,b.cCode ,b.dDate,r.cRdName,'09' as Flag from rdrecords09 a 
left join rdrecord09 b on a.ID=b.ID left join rd_style r on b.cRdCode =r.cRdCode 

insert into #b select AutoID,cBatchProperty6,cBatchProperty7,cBatchProperty8,cBatchProperty9,cbMemo,b.cCode ,b.dDate,r.cRdName,'10' as Flag from rdrecords10 a 
left join rdrecord10 b on a.ID=b.ID left join rd_style r on b.cRdCode =r.cRdCode 

insert into #b select AutoID,cBatchProperty6,cBatchProperty7,cBatchProperty8,cBatchProperty9,cbMemo,b.cCode ,b.dDate,r.cRdName,'11' as Flag from rdrecords11 a 
left join rdrecord11 b on a.ID=b.ID left join rd_style r on b.cRdCode =r.cRdCode 

insert into #b select AutoID,cBatchProperty6,cBatchProperty7,cBatchProperty8,cBatchProperty9,cbMemo,b.cCode ,b.dDate,r.cRdName,'32' as Flag from rdrecords32 a 
left join rdrecord32 b on a.ID=b.ID left join rd_style r on b.cRdCode =r.cRdCode 

insert into #b select AutoID,cBatchProperty6,cBatchProperty7,cBatchProperty8,cBatchProperty9,cbMemo,b.cCode ,b.dDate,r.cRdName,'34' as Flag from rdrecords34 a 
...
,cBatchProperty6 as 物料标识,cBatchProperty7 as 包装批号,cBatchProperty8 as 包装备注,cBatchProperty9 as 作业指导书 ,o.Remark as 检验描述,cCode,i.dDate,cRdName
