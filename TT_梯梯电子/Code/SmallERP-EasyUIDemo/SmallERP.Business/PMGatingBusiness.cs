...
{
    public class PMGatingBusiness
    {
        private readonly PMGatingData dal = new PMGatingData();

        #region  BasicMethod
        /// <summary>
        /// 获得数据列表
        /// </summary>
        public List<PMGatingEntity> DataTableToList(DataTable dt)
        {
            List<PMGatingEntity> modelList = new List<PMGatingEntity>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                PMGatingEntity model;
