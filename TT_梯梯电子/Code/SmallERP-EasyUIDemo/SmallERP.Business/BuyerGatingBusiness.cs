...
namespace SmallERP.Business
{
    public class BuyerGatingBusiness
    {
        ...
        /// <returns></returns>
        public DataTable GetList(string sWhere, SearchAdminDomain domain, out int total, bool b)
        {
            DataTable result = dal.GetList(sWhere, domain.UserId, domain.Name, domain.RoleId, domain.RowStart(), domain.RowEnd(), out total, b);
            return result;
        }
    }
}
