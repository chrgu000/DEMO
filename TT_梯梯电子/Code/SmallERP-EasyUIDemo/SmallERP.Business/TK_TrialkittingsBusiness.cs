...
namespace SmallERP.Business
{
    public class TK_TrialkittingsBusiness
    {
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable GetList(string sTKVersion, string qyPlanner, string qyGroup, string qycategory, string queryName, string qyReorderpolicy, SearchAdminDomain domain, out int total, bool b, bool b1)
        {
            TK_PeriodData tp = new TK_PeriodData();
            DataTable dtt = tp.GetList(sTKVersion);

            PublicBusiness p = new PublicBusiness();
            DataTable dt = dal.GetList(sTKVersion, qyPlanner, qyGroup, qycategory, queryName, qyReorderpolicy, dtt, domain.UserId, domain.Name, domain.RoleId, domain.RowStart(), domain.RowEnd(), out total, b, b1);
