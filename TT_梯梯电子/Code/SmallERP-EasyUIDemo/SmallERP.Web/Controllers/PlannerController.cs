...
namespace SmallERP.Web.Controllers
{
    public class PlannerController : BaseController
    {
        ...
        public ActionResult GetList()
        {
            ...
            string qycategory = Request["qycategory"];
            if (qyVersion != "")
            {
                TK_TrialkittingsBusiness Bll = new TK_TrialkittingsBusiness();
                DataTable result = Bll.GetList(qyVersion, qyPlanner, qyGroup, qycategory, "", qyReorderpolicy, param, out total, true, true);
        ...
        public ActionResult GetQueryList()
        {
            ...
            string querycategory = Request["querycategory"];
            ...
            if (queryPlanner != "")
            {
                sWhere = sWhere + " and default_plannerid='" + queryPlanner + "'";
            }
            if (queryGroup != "")
            {
            ...
            if (querycategory != "")
            {
                sWhere = sWhere + " and category='" + querycategory + "'";
            }
        ...
        public ActionResult GetNetQtyList()
        {
            ...
            if (filterName != "")
            {
                sWhere = sWhere + " and PartID='" + filterName + "'";
            }
            //if (queryPlanner != "")
            //{
        ...
        {
            PublicBusiness Bll = new PublicBusiness();
            object result = Bll.GetcategoryList();
            ...
            try
            {
                ...
                if (files != null && files.Count > 0 && files[0].ContentLength > 0 && !string.IsNullOrEmpty(files[0].FileName))
                {
                    ...
                    if (!string.IsNullOrEmpty(filename))
                    {
                        ...
                        {
                            return Json(new { Msg = "获取上传人账号信息失败", Result = "-1" }, JsonRequestBehavior.AllowGet);
                        }
                        //保存数据库
                        ...
                        if (uploadtype == "0")
                        {
                            #region 上传计算模板
                            TK_Trialkit_Trial_UploadBusiness Bll = new TK_Trialkit_Trial_UploadBusiness();
                            string sTKVersion = Bll.GetNewVersion(LoginId);
                            string sDataVersion = Bll.GetMaxVersion();
                            TK_PeriodBusiness Bllp = new TK_PeriodBusiness();
        ...
        public ActionResult DownloadPAD()
        {
            ...
            {
                NPOIHelper npoi = new NPOIHelper();
                npoi.ExportByWebPAD(qyVersion, qyPlanner, qyGroup, qycategory, qyReorderpolicy, "Sheet1", "PAD-" + qyVersion + ".xlsx", isNetQty);
        ...
        public ActionResult DownloadExcel()
        {
            ...
            {
                TK_TrialkittingsBusiness Bll = new TK_TrialkittingsBusiness();
                DataTable result = Bll.GetList(qyVersion, qyPlanner, qyGroup, qycategory, "", qyReorderpolicy, param, out total, false, true);
