...
namespace SmallERP.DataAccess
{
    public class BuyerGatingData
    {
        ...
        /// <returns></returns>
        public DataTable GetList(string sWhere, string userId, string name, string roleId, int rowStart, int rowEnd, out int total,bool b)
        {
            if (b == true)
            {
                //获得记录总数
            ...
            {
                strSql.Append(@" sProductGroup,sBuyer,iItemNO,dtmDuedate,Period01,Period02,Period03,Period04,Period05,Period06,Period07
                    ,dtmDuedate,Gating,sPONo,fOpenQTY,Vendor,MPN,MFR,Remark,Action ");
            }
            strSql.Append(" FROM ( ");
