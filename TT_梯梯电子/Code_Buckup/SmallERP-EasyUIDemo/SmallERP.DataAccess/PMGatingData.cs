...
namespace SmallERP.DataAccess
{
    public class PMGatingData
    {
        ...
        public DataTable GetList(string sWhere, string userId, string name, string roleId, int rowStart, int rowEnd, out int total)
        {
            ...
            strCountSql.Append(" Where 1=1 group by  sProductGroup,sPartID,dtmRequirement");
            ...
            strSql.Append(" Order By T.sPartID ");
            strSql.Append(")AS Row, sProductGroup as Productgroup,sPartID as Toplevel,dtmRequirement as Reqiesteddate,sum(fOpenQTY) as RequestedQty,convert(nvarchar(4000),'') as  FullkitQty,convert(nvarchar(4000),'') as NonfullkitQty from TK_NetRequirement T ");
            strSql.Append(" WHERE 1=1 group by sProductGroup,sPartID,dtmRequirement");
            if (!string.IsNullOrEmpty(sWhere))
