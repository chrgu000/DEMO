...
namespace SmallERP.DataAccess
{
    public class TK_TrialkittingData
    {
        ...
        {
            if (queryVersion.Substring(0, 3) == "00-")
            {
                StringBuilder strCountSql = new StringBuilder();
                strCountSql.Append(@" Select Count(1) From (select sPartID,dtmRequirement,sum(fQTY) as fQTY from TK_NetRequirement_History T with (nolock) 
left join (select max(sDataVersion) sDataVersion,sTKVersion from TK_Trialkitting_Result with (nolock) group by sTKVersion) R on T.sVersion=R.sDataVersion 
                Where 1=1 group by sPartID,dtmRequirement) T ");
                if (!string.IsNullOrEmpty(sWhere))
                    strCountSql.Replace("1=1", "1=1" + sWhere);
                total = Convert.ToInt32(DbHelperSQL.GetSingle(strCountSql.ToString()));
                //获得记录
                ...
                strSql.Append(@")AS Row, T.* from (select sPartID,dtmRequirement,sum(fQTY) as fQTY from TK_NetRequirement_History T with (nolock) 
left join (select max(sDataVersion) sDataVersion,sTKVersion from TK_Trialkitting_Result with (nolock) group by sTKVersion) R on T.sVersion=R.sDataVersion 
                Where 1=1 group by sPartID,dtmRequirement) T ");
                if (!string.IsNullOrEmpty(sWhere))
                    strSql.Replace("1=1", "1=1" + sWhere);
            ...
            {
                StringBuilder strCountSql = new StringBuilder();
                strCountSql.Append(@" Select Count(1) From (select Partid as sPartID,dtmPeriod as dtmRequirement,sum(Qty) as fQTY from TK_Trialkit_Trial_Upload T with (nolock) 
                Where 1=1 and sTKVersion = '" + queryVersion + "' and  Partid = '" + sPartID + "' group by Partid,dtmPeriod) T ");
                //if (!string.IsNullOrEmpty(sWhere))
                //    strCountSql.Replace("1=1", "1=1" + sWhere);
                total = Convert.ToInt32(DbHelperSQL.GetSingle(strCountSql.ToString()));
                //获得记录
                ...
                strSql.Append(" SELECT ROW_NUMBER() OVER (");
                strSql.Append(" Order By T.sPartID,dtmRequirement ");
                strSql.Append(@")AS Row, T.* from (select Partid as sPartID,dtmPeriod as dtmRequirement,sum(Qty) as fQTY from TK_Trialkit_Trial_Upload T with (nolock) 
                Where 1=1 and sTKVersion = '" + queryVersion + "' and  Partid = '" + sPartID + "' group by Partid,dtmPeriod) T ");
                //if (!string.IsNullOrEmpty(sWhere))
                //    strSql.Replace("1=1", "1=1" + sWhere);
                strSql.Append(" ) TT");
...
left join TK_Trialkitting_Results b on a.sTKVersion=b.sTKVersion where 1=1 group by a.sTKVersion,ProdGroup,Remark,convert(nvarchar(10),dtmCreate,120)
Select Count(1) From 
(
