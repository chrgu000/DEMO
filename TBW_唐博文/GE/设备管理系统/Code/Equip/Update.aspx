<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Update.aspx.cs" Inherits="Equip_Update"
...
<label  id="l1" runat="server" ></label>
    <script language="javascript" type="text/javascript">
        ...
        function doSave() {
            ...
            var icheck = Ajax.EquipAjaxMethod.Check(iid, s1, s2);
            if (icheck != null && icheck.value == true) {
                alert("类型编码或名称重复！");
                return false;
            }

            var obj = Ajax.EquipAjaxMethod.Insert(iid, s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, s14, s15, date1, date2, date3, date4, date5, d1, d2, d3, d4, d5, d6, d7, d8, d9, d10);
        ...
        function doDel() {
            ...
            var isdel = Ajax.EquipAjaxMethod.IsDelete(s1.value);
            if (isdel != null && isdel.value == true) {
                alert("已使用，不可删除！");
                return false;
            }
            
            var obj = Ajax.EquipAjaxMethod.Delete(iid);
            if (obj != null && obj.value==true) {
                window.location.href = "Index.aspx";
                return false;
            }
