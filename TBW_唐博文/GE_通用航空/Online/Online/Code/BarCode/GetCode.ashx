...
public class GetCode : IHttpHandler {
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (context.Request["vCode"] != null)
            _codeText = context.Request["vCode"].ToString().Trim();
        if (context.Request["m"] != null)
            _Magnify = context.Request["m"].ToString();
        if (context.Request["h"] != null)
            _height = context.Request["h"].ToString();
        if (context.Request["f"] != null && context.Request["f"].ToString()!="")
            _font = float.Parse(context.Request["f"].ToString());
        ...
        this.ViewFont = new Font("宋体", _font);
        Code39();
        //输出img
