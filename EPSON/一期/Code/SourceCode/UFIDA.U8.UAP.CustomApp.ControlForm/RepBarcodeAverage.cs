...
{
    public partial class RepBarcodeAverage : UserControl
    {
        ...
        string sPrintLayOutMod = Application.StartupPath + "\\UAP\\Runtime\\print\\RepBarcodeAverage.dll";
...
SELECT CUSTOMER,cCusCode,Department,PARTCODE,sum(offph)/count(*) AS TOTALOFFPH,sum(tat)/count(*) AS TOTALTIME
FROM #a 
group by CUSTOMER,Department,PARTCODE,cCusCode
ORDER BY Department, CUSTOMER,PARTCODE
