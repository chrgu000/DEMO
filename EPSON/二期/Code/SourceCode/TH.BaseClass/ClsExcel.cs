...
namespace TH.BaseClass
{
    ...
    public class ClsExcel
    {
        ...
        public DataSet ExcelToDS(string Path, ArrayList arrList,bool bIsTitle)
        {
            try
            {
                string strConn = "Provider=Microsoft.ACE.OLEDB.12.0;" + "Data Source=" + Path + ";" + "Extended Properties='Excel 8.0;IMEX=1;";
                if (bIsTitle)
                    strConn += "HDR=YES;'";
                else
                    strConn += "HDR=NO;'";
        ...
        public DataTable GetSheetName(string filePath)
        {
            try
            {
                string strConn = string.Format("Provider=Microsoft.Ace.OLEDB.12.0;Data Source={0};Extended Properties='Excel 12.0;HDR=Yes;IMEX=2'", filePath);
