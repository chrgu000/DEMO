...
{
    public partial class PendingQC : UserControl
    {
...
where 1=1
order by a.iSOsID,a.BarCode
";
            if (radioALL.Checked)
            {
                sSQL = sSQL.Replace("1=1", @"1=1 and a.Process in (select cWhCode from Warehouse where cWhMemo like '%IQC%' OR cWhMemo like '%OQC%') 
	and (a.IQCStatus = 'Pending IQC' or a.OQCStatus = 'Pending OQC')"
                    
                    );
            }
            if (radioIQC.Checked)
            {
                sSQL = sSQL.Replace("1=1", @"1=1 and a.Process in (select cWhCode from Warehouse where cWhMemo like '%IQC%') 
	and (a.IQCStatus = 'Pending IQC' )");
            }
            if (radioOQC.Checked)
            { 
                sSQL = sSQL.Replace("1=1", @"1=1 and a.Process in (select cWhCode from Warehouse where cWhMemo like '%OQC%') 
	and (a.IQCStatus = 'Pending OQC' )");
            }
