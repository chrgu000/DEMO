...
{
    public partial class PurBillVouch : UserControl
    {
        ...
        {
            string sSQL = "select cPBVCode from PurBillVouch  order by cPBVCode";
...
";
                ...
                for (int i = 0; i < dtDetails.Rows.Count; i++)
                {
                    ...
                    //7 Revised Indicator
                    decimal diOriMoney = BaseFunction.ReturnDecimal(dtDetails.Rows[i]["iMoney"]);
                    if (diOriMoney > 0)
                    {
                        strDetail.Append("A");
                    }
