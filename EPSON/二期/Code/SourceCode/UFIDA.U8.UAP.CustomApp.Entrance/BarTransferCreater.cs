...
{
    public class BarTransferCreater : INetUserControl
    {
        ...
        System.Windows.Forms.Control INetUserControl.CreateControl(UFSoft.U8.Framework.Login.UI.clsLogin login, string MenuID, string Paramters)
        {
            ...
            if (clsUser.chkRight(login, "TH_14"))
            {
                if (DateTime.Now > Convert.ToDateTime("2020-7-06"))
                {
                    return null;
                }

                BarTransfer fm = new BarTransfer();
                fm.Conn = conn;
