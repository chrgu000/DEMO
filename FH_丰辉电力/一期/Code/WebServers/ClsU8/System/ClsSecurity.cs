...
namespace ClsU8
{
    public class ClsSecurity
    {
        ...
        string Code = "cSCode";
        string Name = "cSName";
        string CodeTitle = "安全标准档案编码";
        string NameTitle = "安全标准档案名称";
        string sSQL = "";
            ...
            try
            {
                ...
                for (int i = 0; i < dtGrid.Rows.Count; i++)
                {
                    if (dtGrid.Rows[i]["iSave"].ToString().Trim() == "update" || dtGrid.Rows[i]["iSave"].ToString().Trim() == "add")
                    {
                        ...
                        if (dtGrid.Rows[i]["Score"].ToString().Trim() == "")
                        {
                            sErr = sErr + "行" + (i + 1)  + "分值不能为空\n";
                            continue;
                        }
                        #endregion
                        ...
                        dr["Score"] = dtGrid.Rows[i]["Score"].ToString().Trim();
        ...
        public string del(string scSCode)
        {
            string sErr = "";
            ArrayList aList = new ArrayList();
            try
            {
                sSQL = "select count(1) from dbo.ProjectSecurity where " + Code + " = '" + scSCode + "' ";
                long iCou = Convert.ToInt64(clsSQLCommond.ExecGetScalar(sSQL));
                if (iCou > 0)
                {
                    throw new Exception("安全档案 " + scSCode + " 在工程中已经使用不能删除\n");
                }

                sSQL = "delete " + tablename + " where " + Code + " = '" + scSCode + "' ";
                aList.Add(sSQL);
        ...
        public bool bClosed(string cSCode)
        {
            string sErr = "";
            ArrayList aList = new ArrayList();
            try
            {
                sSQL = "select bClosed from " + tablename + " where " + Code + "='" + cSCode + "' ";
