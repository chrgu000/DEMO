...
namespace ClsU8
{
    ...
    {
        string tablename = "Quality";
        string Code = "cQCode";
        string Name = "cQName";
        string CodeTitle = "质量标准档案编码";
        string NameTitle = "质量标准档案名称";
        string sSQL = "";
        ...
        public string dt()
        {
            ...
            try
            {
                sSQL = "select *, 'edit' as iSave from Quality order by cQCode";
        ...
        public string del(string scQCode)
        {
            string sErr = "";
            ArrayList aList = new ArrayList();
            try
            {
                sSQL = "select count(1) from dbo.ProjectQuality where " + Code + " = '" + scQCode + "' ";
                long iCou = Convert.ToInt64(clsSQLCommond.ExecGetScalar(sSQL));
                if (iCou > 0)
                {
                    throw new Exception("质量档案 " + scQCode + " 在工程中已经使用不能删除\n");
                }

                sSQL = "delete " + tablename + " where " + Code + " = '" + scQCode + "' ";
                aList.Add(sSQL);
