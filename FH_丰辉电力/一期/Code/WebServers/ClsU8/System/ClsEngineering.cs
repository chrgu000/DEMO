...
namespace ClsU8
{
    public class ClsEngineering
    {
        string tablename = "Engineering";
        string Code = "cECode";
        string Name = "cEName";
        string CodeTitle = "工程性质档案编码";
        string NameTitle = "工程性质档案名称";
        string sSQL = "";
        ...
        public string del(string scECode)
        {
            string sErr = "";
            ArrayList aList = new ArrayList();
            try
            {
                sSQL = "select count(1) from dbo.Project where " + Code + " = '" + scECode + "' ";
                long iCou = Convert.ToInt64(clsSQLCommond.ExecGetScalar(sSQL));
                if (iCou > 0)
                {
                    throw new Exception("工程性质档案 " + scECode + " 在工程中已经使用不能删除\n");
                }

                sSQL = "delete " + tablename + " where " + Code + " = '" + scECode + "' ";
                aList.Add(sSQL);
