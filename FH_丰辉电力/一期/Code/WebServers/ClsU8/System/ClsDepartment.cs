...
namespace ClsU8
{
    public class ClsDepartment
    {
        ...
        string Code = "cDepCode";
        string Name = "cDepName";
        string CodeTitle = "部门编码";
        string NameTitle = "部门名称";
        string sSQL = "";
        ...
        public string del(string sDel)
        {
            ...
            try
            {
                ...
                sSQL = "select count(1) from dbo.Person where " + Code + " = '" + sDel + "' ";
                long iCou = Convert.ToInt64(clsSQLCommond.ExecGetScalar(sSQL));
                if (iCou > 0)
                {
                    sErr = "部门编码：" + sDel + " 已经使用不能删除\n";
                }

                sSQL = "delete " + tablename + " where " + Code + " = '" + sDel + "' ";
                aList.Add(sSQL);
                ...
                if (aList.Count > 0)
                {
                    int iCouDel = clsSQLCommond.ExecSqlTran(aList);
                }
            }
            catch (Exception ee)
            {
