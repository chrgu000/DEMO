...
namespace ClsU8
{
    public class ClsInventory
    {
        ...
        string CodeTitle = "存货档案编码";
        string NameTitle = "存货档案名称";
        string sSQL = "";
            ...
            try
            {
                ...
                for (int i = 0; i < dtGrid.Rows.Count; i++)
                {
                    if (dtGrid.Rows[i]["iSave"].ToString().Trim() == "update" || dtGrid.Rows[i]["iSave"].ToString().Trim() == "add")
                    {
                        ...
                        if (dtGrid.Rows[i]["cComunitCode"].ToString().Trim() == "")
                        {
                            sErr = sErr + "行" + (i + 1)  + "计量单位不能为空\n";
                            continue;
                        }
                        #endregion
                        ...
                        dr["cInvStd"] = dtGrid.Rows[i]["cInvStd"].ToString().Trim();
                        dr["cComunitCode"] = dtGrid.Rows[i]["cComunitCode"].ToString().Trim();
        ...
        public string del(string cInvCode)
        {
            string sErr = "";
            ArrayList aList = new ArrayList();
            try
            {
                sSQL = "select count(1) from dbo.ProjectRecord where " + Code + " = '" + cInvCode + "' ";
                long iCou = Convert.ToInt64(clsSQLCommond.ExecGetScalar(sSQL));
                if (iCou > 0)
                {
                    throw new Exception("存货档案 " + cInvCode + " 在工程中已经使用不能删除\n");
                }

                sSQL = "select count(1) from dbo.RdRecords where " + Code + " = '" + cInvCode + "' ";
                iCou = Convert.ToInt64(clsSQLCommond.ExecGetScalar(sSQL));
                if (iCou > 0)
                {
                    throw new Exception("存货档案 " + cInvCode + " 在仓库盘点中已经使用不能删除\n");
                }

                sSQL = "delete " + tablename + " where " + Code + " = '" + cInvCode + "' ";
                aList.Add(sSQL);
        ...
        public bool bClosed(string cInvCode)
        {
            string sErr = "";
            ArrayList aList = new ArrayList();
            try
            {
                sSQL = "select bClosed from " + tablename + " where " + Code + "='" + cInvCode + "' ";
