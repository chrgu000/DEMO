...
namespace ClsU8
{
    public class ClsDistrictClass
    {
        string tablename = "DistrictClass";
        string Code = "cDCCode";
        string Name = "cDCName";
        string CodeTitle = "地区编码";
        string NameTitle = "地区名称";
        string sSQL = "";
        ...
        public string del(string sPersonCode)
        {
            string sErr = "";
            ArrayList aList = new ArrayList();
            try
            {
                sSQL = "select count(1) from dbo.Project where " + Code + " = '" + sPersonCode + "' ";
                long iCou = Convert.ToInt64(clsSQLCommond.ExecGetScalar(sSQL));
                if (iCou > 0)
                {
                    throw new Exception("部门 " + sPersonCode + " 在工程中已经使用不能删除\n");
                }

                sSQL = "delete " + tablename + " where " + Code + " = '" + sPersonCode + "' ";
                aList.Add(sSQL);
