...
namespace TH.WebService.DAL
{
    ...
    public partial class RdRecord09
    {
        ...
        /// 获得数据列表
        /// </summary>
        public DataTable GetRdrecord09(string sCode)
        {
...
";
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public int iSaveRdrecord09ChkQty(string sCode, DataTable dtBarCode,string sUid)
        {
            int iCou = 0;
            try
            {
                ...
                try
                {
                    ...
                    string sSQL = "select a.cCode,b.iRowNo, b.cInvCode,b.iQuantity,b.cDefine29 as 已扫描数量,b.cDefine28 as 货位,b.cDefine30 as 批号,cast(null as decimal(16,6)) as 本次扫描数量,b.autoid from RdRecord09 a inner join rdrecords09 b on a.id = b.id where a.cCode = '" + sCode + "' order by b.cInvCode";
                    DataTable dtRdRecord09 = DbHelperSQL.ExecuteDataset(tran, CommandType.Text, sSQL).Tables[0];
                    ...
                    for (int i = 0; i < dtBarCode.Rows.Count; i++)
                    {
                        ...
                        //条形码中仅存货编码与入库单对应
                        DataRow[] drCode = dtRdRecord09.Select("cInvCode = '" + sInvCode + "'");
                        ...
                        for (int j = 0; j < drCode.Length; j++)
                        {
                            ...
                            sSQL = "update rdrecords09 set cDefine29 = cast(isnull(cDefine29,0) as decimal(16,6)) + " + model_BarCodeRD.Qty.ToString() + " 111111 where autoid = " + model_BarCodeRD.ExsID.ToString();
                            if (sUpdate == "")
...
";
                    ...
                    if (dtTemp2 != null && dtTemp2.Rows.Count == 1)
                    {
                        ...
                        if (iC == 0)
                        {
                            sSQL = "update rdrecord09 set cHandler = '222222',dVeriDate  = '333333',dnverifytime = getdate() where cCode = '111111'";
                            sSQL = sSQL.Replace("111111", sCode);
