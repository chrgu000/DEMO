...
{
    public partial class PrintBarCode_RdRecord01 : UserControl
    {
        TH.BaseClass.GetBaseData getBaseData = new GetBaseData();
        DAL_BarCodeList DAL = new DAL_BarCodeList();
        string sProPath = Application.StartupPath;

        string sPrintLayOutMod = Application.StartupPath + "\\UAP\\Runtime\\print\\Model\\采购入库条形码打印Mod.dll";
        string sPrintLayOutUser = Application.StartupPath + "\\UAP\\Runtime\\print\\User\\采购入库条形码打印User.dll";
        ...
        public PrintBarCode_RdRecord01()
        {
            InitializeComponent();
        }
...
select cast(0 as bit) as choose
    ,a.cCode,a.dDate,a.cBusType,a.cVenCode,e.cVenName,a.cDepCode,c.cDepName,a.cWhCode,d.cWhName,a.cMaker,a.cHandler
	,b.cInvCode,f.cInvName,f.cInvStd,b.iQuantity,b.cDefine27 as 包装批量,b.cDefine28 as 货位,b.cDefine30 as 批号
    ,b.AutoID,b.irowno,g.PrintQty,g.PrintNum
    ,b.cPOID,b.iPOsID ,i.cPoid,h.ivouchrowno as PORowNo,a.cPTCode,l.cPTName 
    ,f.cInvDefine3,m.cComUnitName
    ,a.cMemo,f.cInvDefine4,f.cInvDefine5
from RdRecord01 a
    ...
    left join (select sum(iQty) as PrintQty,count(1) as PrintNum,ExsID from _BarCodeList group by ExsID) g on g.ExsID = b.AutoID
    left join PO_Podetails h on h.id = b.iPOsID 
    left join Po_pomain i on i.poid = h.poid
    left join PurchaseType l on l.cPTCode = a.cPTCode
	left join ComputationUnit m on m.cComunitCode = f.cComUnitCode
where 1=1
...
";
                ...
                if (!chkScanIn.Checked)
                {
                    sSQL = sSQL.Replace("1=1", "1=1 and b.iQuantity <> isnull(b.cDefine29,0)");
                }
