...
{
    public partial class PrintBarCode_RdRecord08 : UserControl
    {
        TH.BaseClass.GetBaseData getBaseData = new GetBaseData();
        DAL_BarCodeList DAL = new DAL_BarCodeList();
        string sProPath = Application.StartupPath;

        string sPrintLayOutMod = Application.StartupPath + "\\UAP\\Runtime\\print\\Model\\其他入库条形码打印Mod.dll";
        string sPrintLayOutUser = Application.StartupPath + "\\UAP\\Runtime\\print\\User\\其他入库条形码打印User.dll";
        ...
        public PrintBarCode_RdRecord08()
        {
            InitializeComponent();
        }
                    ...
                    for (int i = 0; i < gridView1.RowCount; i++)
                    {
                        if (BaseFunction.ReturnBool(gridView1.GetRowCellValue(i,gridColchoose)))
                        {
                            ...
                            for (int j = 0; j < iCount; j++)
                            {
                                ...
                                model.Batch = gridView1.GetRowCellValue(i, gridCol批号).ToString().Trim();
        ...
        {
            string sSQL = @"
select distinct cCode,dDate from RdRecord08 order by dDate,cCode
";
...
from RdRecord08 a
    ...
    left join (select sum(iQty) as PrintQty,ExsID,COUNT(1) as PrintNum from _BarCodeList group by ExsID) g on g.ExsID = b.AutoID
where 1=1
...
";
                ...
                if (!chkRd08.Checked)
                {
                    sSQL = sSQL.Replace("1=1", "1=1 and isnull(a.cDefine1,'') = '是'");
                }
