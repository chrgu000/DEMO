...
namespace 成衣生产.DAL
{
    ...
    public partial class 生产计划
    {
        ...
        /// 增加一条数据
        /// </summary>
        public string Add(成衣生产.Model.生产计划 model)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            if (model.订单iID != null)
            {
                strSql1.Append("订单iID,");
                strSql2.Append("" + model.订单iID + ",");
            }
            if (model.单据日期 != null)
            {
                strSql1.Append("单据日期,");
            ...
            if (model.计划生产日期 != DateTime.MinValue)
            {
                strSql1.Append("计划生产日期,");
                strSql2.Append("'" + model.计划生产日期 + "',");
            }
            if (model.生产日期 != DateTime.MinValue)
            {
                strSql1.Append("生产日期,");
                strSql2.Append("'" + model.生产日期 + "',");
            }
            if (model.入库日期 != DateTime.MinValue)
            {
                strSql1.Append("入库日期,");
                strSql2.Append("'" + model.入库日期 + "',");
            }
            if (model.完成日期 != DateTime.MinValue)
            {
                strSql1.Append("完成日期,");
                strSql2.Append("'" + model.完成日期 + "',");
            }
            if (model.联系电话 != null)
            {
                strSql1.Append("联系电话,");
        ...
        /// 更新一条数据
        /// </summary>
        public string Update(成衣生产.Model.生产计划 model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update 生产计划 set ");
            if (model.订单iID != null)
            {
                strSql.Append("订单iID=" + model.订单iID + ",");
            }
            if (model.单据日期 != null)
            {
            ...
            if (model.计划生产日期 != DateTime.MinValue)
            {
                strSql.Append("计划生产日期='" + model.计划生产日期 + "',");
            }
            else
            {
                strSql.Append("计划生产日期= null ,");
            }
            if (model.生产日期 != DateTime.MinValue)
            {
                strSql.Append("生产日期='" + model.生产日期 + "',");
            }
            else
            {
                strSql.Append("生产日期= null ,");
            }
            if (model.入库日期 != DateTime.MinValue)
            {
                strSql.Append("入库日期='" + model.入库日期 + "',");
            }
            else
            {
                strSql.Append("入库日期= null ,");
            }
            if (model.完成日期 != DateTime.MinValue)
            {
                strSql.Append("完成日期='" + model.完成日期 + "',");
            }
            else
            {
                strSql.Append("完成日期= null ,");
            }
            if (model.联系电话 != null)
            {
        ...
        /// 得到一个对象实体
        /// </summary>
        public 成衣生产.Model.生产计划 DataRowToModel(DataRow row)
        {
            成衣生产.Model.生产计划 model = new 成衣生产.Model.生产计划();
            if (row != null)
            {
                ...
                if (row["订单iID"] != null && row["订单iID"].ToString() != "")
                {
                    model.订单iID = int.Parse(row["订单iID"].ToString());
                }
                if (row["单据日期"] != null && row["单据日期"].ToString() != "")
                {
                    model.单据日期 = DateTime.Parse(row["单据日期"].ToString());
                }
                if (row["计划生产日期"] != null && row["计划生产日期"].ToString() != "")
                {
                    model.计划生产日期 = DateTime.Parse(row["计划生产日期"].ToString());
                }
                if (row["生产日期"] != null && row["生产日期"].ToString() != "")
                {
                    model.生产日期 = DateTime.Parse(row["生产日期"].ToString());
                }
                if (row["入库日期"] != null && row["入库日期"].ToString() != "")
                {
                    model.入库日期 = DateTime.Parse(row["入库日期"].ToString());
                }
                if (row["完成日期"] != null && row["完成日期"].ToString() != "")
                {
                    model.完成日期 = DateTime.Parse(row["完成日期"].ToString());
                }
                if (row["联系电话"] != null)
                {
