...
namespace 基础设置
{
    public partial class Frm采购类型 : 系统服务.FrmBaseInfo
    {
        string tablename = "PurchaseType";
        ...
        private void btnSave()
        {
            ...
            for (int i = 0; i < gridView1.RowCount; i++)
            {
                if (gridView1.GetRowCellDisplayText(i, gridColiSave).ToString().Trim() == "update" || gridView1.GetRowCellDisplayText(i, gridColiSave).ToString().Trim() == "add")
                {
                    ...
                    dr["cPTCode"] = gridView1.GetRowCellValue(i, gridColCode).ToString().Trim();
                    dr["cPTName"] = gridView1.GetRowCellValue(i, gridColName).ToString().Trim();
        ...
        private void Frm采购类型_Load(object sender, EventArgs e)
        {
            try
            {
