...
namespace 基础设置
{
    public partial class Frm存货档案比对 : 系统服务.FrmBaseInfo
    {
        string tablename = "InventoryContrast";
            ...
            if (result == DialogResult.OK)
            {
                try
                {
                    ...
                    for (int i = 0; i < gridView1.RowCount; i++)
                    {
                        if (gridView1.IsRowSelected(i))
                        {
                            string sID = gridView1.GetRowCellValue(i, gridColcInvCode).ToString().Trim();
                            sSQL = "select count(1) from dbo.PO_POmain where cPTCode = '" + sID + "' ";
        ...
        private void btnSave()
        {
            ...
            for (int i = 0; i < gridView1.RowCount; i++)
            {
                ...
                {
                    #region 判断
                    if (gridView1.GetRowCellDisplayText(i, gridColcInvCode).ToString().Trim() == "")
                    {
                        continue;
                    }
                    if (gridView1.GetRowCellDisplayText(i, gridColcInvCode).ToString().Trim() == "")
                    {
                        sErr = sErr + "行" + (i + 1) + gridColcInvCode.Caption + "不能为空\n";
                        continue;
                    }
                    #endregion
                    ...
                    for (int j = 0; j < i; j++)
                    {
                        if (gridView1.GetRowCellDisplayText(i, gridColcInvCode).ToString().Trim() == gridView1.GetRowCellDisplayText(j, gridColcInvCode).ToString().Trim())
                        {
                            sErr = sErr + "行" + (i + 1) + gridColcInvCode.Caption + "重复\n";
                            continue;
                        }
                    }
                    #endregion
                    ...
                    dr["cInvCode"] = gridView1.GetRowCellValue(i, gridColcInvCode).ToString().Trim();

                    dr["cInvNewCode"] = gridView1.GetRowCellValue(i, gridColcInvNewCode).ToString().Trim();
                    dr["cInvNewName"] = gridView1.GetRowCellValue(i, gridColcInvNewName).ToString().Trim();
        ...
        private void Frm存货档案比对_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void SetLookUpEdit()
        {
            ...
            系统服务.LookUp.Inventory(ItemLookUpEdit存货);
            ItemLookUpEdit存货.Properties.DisplayMember = "cInvName";
        }
        ...
        private void gridView1_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            try
            {
                ...
                if (e.Column == gridColcInvCode)
                {
                    gridView1.SetRowCellValue(iRow, gridColcInvNewCode, gridView1.GetRowCellValue(iRow, gridColcInvCode));
                    gridView1.SetRowCellValue(iRow, gridColcInvNewName, gridView1.GetRowCellValue(iRow, gridColcInvCode));
                }
                ...
                if (e.Column == gridColcInvCode && iRow == gridView1.RowCount - 1 && gridView1.GetRowCellDisplayText(iRow, gridColcInvCode).ToString().Trim() != "")
                {
                    gridView1.AddNewRow();
        ...
        private void gridView1_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    gridColcInvCode.OptionsColumn.AllowEdit = true;
                }
                else
                {
                    gridColcInvCode.OptionsColumn.AllowEdit = false;
                }
            }
            catch { }
        }
        ...
        private void ItemButtonEdit存货_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            int iRow = 0;
            if (gridView1.FocusedRowHandle > 0)
            ...
            {
                gridView1.SetRowCellValue(iRow, gridColcInvCode, frm.sID);
                frm.Enabled = true;
            }
        }
