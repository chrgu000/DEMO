...
namespace 基础设置
{
    public partial class Frm货位档案 : 系统服务.FrmBaseInfo
    {
        string tablename = "Position";
        string tableid = "cPosCode";
        public Frm货位档案()
        {
            InitializeComponent();
            ...
            if (result == DialogResult.OK)
            {
                try
                {
                    ...
                    for (int i = 0; i < gridView1.RowCount; i++)
                    {
                        if (gridView1.IsRowSelected(i))
                        {
                            string sID = gridView1.GetRowCellValue(i, gridCol货位编码).ToString().Trim();
                            sSQL = "select count(1) from dbo.RdRecords where cPosCode = '" + sID + "' ";
                            ...
                            sSQL = "select count(1) from dbo.RdRecords13 where cPosCode = '" + sID + "' ";
        ...
        private void btnSave()
        {
            try
            {
                gridView1.FocusedColumn = gridCol货位编码;
                gridView1.FocusedRowHandle -= 1;
                gridView1.FocusedRowHandle += 1;
                gridView1.Focus();
            }
            ...
            for (int i = 0; i < gridView1.RowCount; i++)
            {
                ...
                {
                    #region 判断
                    if (gridView1.GetRowCellDisplayText(i, gridCol货位编码).ToString().Trim() == ""
                        && gridView1.GetRowCellValue(i, gridCol货位名称).ToString().Trim() == "")
                    {
                        continue;
                    }
                    if (gridView1.GetRowCellDisplayText(i, gridCol货位编码).ToString().Trim() == "")
                    {
                        sErr = sErr + "行" + (i + 1) + gridCol货位编码.Caption + "不能为空\n";
                        continue;
                    }

                    if (gridView1.GetRowCellValue(i, gridCol货位名称).ToString().Trim() == "")
                    {
                        sErr = sErr + "行" + (i + 1) + gridCol货位名称.Caption + "不能为空\n";
                        continue;
                    }
                    #endregion
                    ...
                    for (int j = 0; j < i; j++)
                    {
                        if (gridView1.GetRowCellDisplayText(i, gridCol货位编码).ToString().Trim() == gridView1.GetRowCellDisplayText(j, gridCol货位编码).ToString().Trim())
                        {
                            sErr = sErr + "行" + (i + 1) + gridCol货位编码.Caption + "重复\n";
                            continue;
                        }
                    }
                    ...
                    for (int j = 0; j < i; j++)
                    {
                        if (gridView1.GetRowCellDisplayText(i, gridCol货位名称).ToString().Trim() == gridView1.GetRowCellDisplayText(j, gridCol货位名称).ToString().Trim())
                        {
                            sErr = sErr + "行" + (i + 1) + gridCol货位名称.Caption + "重复\n";
                            continue;
                        }
                    }
                    #endregion
                    ...
                    dr["cPosCode"] = gridView1.GetRowCellValue(i, gridCol货位编码).ToString().Trim();
                    dr["cPosName"] = gridView1.GetRowCellValue(i, gridCol货位名称).ToString().Trim();
        ...
        private void Frm货位档案_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        {
            系统服务.LookUp.Warehouse(ItemLookUpEdit仓库编码);
        }
        ...
        private void gridView1_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            try
            {
                ...
                if (e.Column == gridCol货位名称 && e.RowHandle == gridView1.RowCount - 1 && gridView1.GetRowCellDisplayText(e.RowHandle, gridCol货位编码).ToString().Trim() != "")
                {
        ...
        private void gridView1_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            try
            {
                ...
                {
                    gridCol货位编码.OptionsColumn.AllowEdit = true;
                }
                else
                {
                    gridCol货位编码.OptionsColumn.AllowEdit = false;
                }
            }
            catch { }
        }
