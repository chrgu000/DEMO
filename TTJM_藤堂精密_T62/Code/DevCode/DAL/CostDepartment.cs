...
namespace TH.DAL
{
    ...
    /// 数据访问类:基础档案列表
    /// </summary>
    public partial class CostDepartment
    {
        ...
        {
            string sSQL = @"
select a.sCode,b.sText,cast(0 as bit) as bChoose, b.iID,b.Guid,b.sType,b.sTextEn,b.sText2,b.sText3,b.sText4,b.sText5,b.sRemark,b.sRemark2,b.sRemark3,b.sRemark4,b.sRemark5,b.CreateUid,b.CreateDate,b.UpdateUid,b.UpdateDate,b.CloseUid,b.CloseDate
from
(
	select sCode,sText from _BaseDataList where isnull(CloseUid,'') = '' and sType = '成本中心'
)a
left join 
(
	select * from _BaseDataList where isnull(CloseUid,'') = '' and sType = '成本中心对照'
)b on a.sCode = b.sCode
111111
order by a.sCode
";
            if (strWhere.Trim() != "")
            {
                sSQL = sSQL.Replace("111111", " where " + strWhere);
            }
            else
            {
                ...
                try
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        if (BaseFunction.BaseFunction.ReturnBool(dt.Rows[i]["bChoose"]))
                        {
                            ...
                            string sSQL = "select * from _BaseDataList where sCode = '" + model.sCode + "' and sType = '成本中心'";
                            DataTable dtTemp = DbHelperSQL.Query(sSQL);
                            if (dtTemp == null || dtTemp.Rows.Count == 0)
                            {
                                sErr = sErr + "成本中心 " + model.sCode + " 已经删除\n";
                                continue;
                            }
