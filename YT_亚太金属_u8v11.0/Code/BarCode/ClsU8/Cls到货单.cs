...
namespace ClsU8
{
    public class Cls到货单
    {
        ...
        public string dt到货单信息(string sCode)
        {
            ...
            try
            {
                string sSQL = "select * from PU_ArrivalVouch a inner join PU_ArrivalVouchs b on a.ID  = b.ID  where a.cCode  = '" + sCode.Trim() + "' ";
        ...
        public string dt到货单明细信息(string sCode, string sWhere)
        {
            ...
            {
                string sSQL = @"
select b.* ,cInvName,cInvStd,c.cDefWareHouse,c.cPosition,c.fInExcess,d.Autoid as 条形码,bInvBatch as 是否批次管理 
