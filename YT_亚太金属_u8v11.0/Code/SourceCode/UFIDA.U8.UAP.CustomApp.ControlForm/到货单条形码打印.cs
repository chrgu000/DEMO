...
{
    public partial class 到货单条形码打印 : UserControl
    {
...
where ISNULL(a.ccloser ,'') = '' and ISNULL(a.cverifier ,'') <> '' and isnull(h.AutoID,0) = 0
    and 1=1
order by b.autoid
...
from PU_ArrivalVouch a inner join PU_ArrivalVouchs b on a.id = b.id
    ...
    left join _BarCodeInventory h on h.cInvCode = b.cInvCode and ISNULL(h.cfree1,'') = ISNULL(b.cfree1,'') and ISNULL(h.cfree2,'') = ISNULL(b.cfree2,'') and ISNULL(h.cfree3,'') = ISNULL(b.cfree3,'') and ISNULL(h.cfree4,'') = ISNULL(b.cfree4,'')
		 and ISNULL(h.cfree5,'') = ISNULL(b.cfree5,'') and ISNULL(h.cfree6,'') = ISNULL(b.cfree6,'') and ISNULL(h.cfree7,'') = ISNULL(b.cfree7,'') and ISNULL(h.cfree8,'') = ISNULL(b.cfree8,'') and ISNULL(h.cfree9,'') = ISNULL(b.cfree9,'') and ISNULL(h.cfree10,'') = ISNULL(b.cfree10,'')
		 and ISNULL(h.cbatch,'') = ISNULL(b.cbatch,'')
where ISNULL(a.ccloser ,'') = '' and ISNULL(a.cverifier ,'') <> ''
    and 1=1
order by b.autoid
";
                ...
                if (textEdit供应商.Text.Trim() != "")
                {
                    sSQL = sSQL.Replace("1=1", "1=1 and (a.cVenCode like '%" + textEdit供应商.Text.Trim() + "%' or g.cVenName like '%" + textEdit供应商.Text.Trim() + "%')");
                }
                if (lookUpEdit所属公司.Text.Trim() != "")
                {
                    sSQL = sSQL.Replace("1=1", "1=1 and a.cDefine3 = '" + lookUpEdit所属公司.Text.Trim() + "'");
                }
                if (textEdit合同号.Text.Trim() != "")
                {
                    sSQL = sSQL.Replace("1=1", "1=1 and a.cDefine1 like '%" + textEdit合同号.Text.Trim() + "%'");
                }
                if (radio未完成.Checked)
                {
                    sSQL = sSQL.Replace("1=1", "1=1 and b.iQuantity > ISNULL(b.fInspectQuantity,0) and b.iQuantity > ISNULL(b.fValidInQuan,0)");
                }
        ...
        {
            string sSQL = @"select cCode from PU_ArrivalVouch order by cCode";
            DataTable dt = SqlHelper.ExecuteDataset(Conn, CommandType.Text, sSQL).Tables[0];
            lookUpEdit单据号1.Properties.DataSource = dt;
            lookUpEdit单据号2.Properties.DataSource = dt;

            sSQL = "select cValue from UserDefine where cid = '03'";
            dt = SqlHelper.ExecuteDataset(Conn, CommandType.Text, sSQL).Tables[0];
            lookUpEdit所属公司.Properties.DataSource = dt;
        }
        ...
        private void tsbClearCon_Click(object sender, EventArgs e)
        {
            ...
            lookUpEdit所属公司.EditValue = DBNull.Value;
            textEdit供应商.Text = "";
            textEdit合同号.Text = "";
            dateEdit单据日期1.Text = "";
            dateEdit单据日期2.Text = "";
