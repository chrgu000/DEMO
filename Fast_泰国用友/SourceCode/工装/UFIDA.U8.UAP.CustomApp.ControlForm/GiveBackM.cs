...
{
    public partial class GiveBackM : UserControl
    {
        ...
        {
            string sSQL = @"
select cCode from _GiveBackM order by cCode
";
...
";
        ...
        private void btnExcel_Click(object sender, EventArgs e)
        {
            try
            {
                if (!clsUserRight.chkRight(sUserID, 5590))
...
from [dbo].[_GiveBackM] a inner join [dbo].[_GiveBackMs] b on a.[cCode] = b.cCode
where 1=1
...
";
        ...
        private void btnSEL_Click(object sender, EventArgs e)
        {
            try
            {
                if (!clsUserRight.chkRight(sUserID, 5510))
                    throw new Exception("没有权限");
        ...
        private void btnDEL_Click(object sender, EventArgs e)
        {
            try
            {
                if (!clsUserRight.chkRight(sUserID, 5540))
                    throw new Exception("没有权限");
                string sErr = "";
                try
                {
                ...
                try
                {
                        ...
                        string sSQL = "select * from  _GiveBackM where cCode = N'111111'";
                        ...
                        sSQL = "update _FrockClamp set iState = 2 where SerialNo in (select SerialNo from _Maintenances where cCode = N'111111')";
                        sSQL = sSQL.Replace("111111", sCode);
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                if (!clsUserRight.chkRight(sUserID, 5520))
                    throw new Exception("没有权限");

                GiveBackM_Edit frm = new GiveBackM_Edit(Conn, "");
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            try
            {
                if (!clsUserRight.chkRight(sUserID, 5530))
                    throw new Exception("没有权限");
                ...
                GiveBackM_Edit frm = new GiveBackM_Edit(Conn, sCode);
                frm.sUserName = this.sUserName;
                frm.sUserID = this.sUserID;
                frm.StartPosition = FormStartPosition.CenterParent;
                frm.ShowDialog();
